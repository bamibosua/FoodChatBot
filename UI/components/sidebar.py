        ...
        with st.expander("Chat History", expanded=True):
            if st.session_state.get('all_chats'): # kiem tra all chats co ton tai, neu co thi hien thi, neu khong thi hien thi thong bao
                ...
                for chat_id, data in ordered[:10]:  # Show only latest 10
                    ...
                    # with c2:
                    #     # Favorite toggle
                    #     is_fav = chat_id in st.session_state.get('favorites', [])
                    #     # if is_fav, show filled star, else show empty star
                    #     if st.button("⭐" if is_fav else "☆", key=f"fav_{chat_id}", use_container_width=True):
                    #         if 'favorites' not in st.session_state:
                    #             st.session_state.favorites = []
                    #         if is_fav:
                    #             st.session_state.favorites.remove(chat_id)
                    #         else:
                    #             st.session_state.favorites.append(chat_id)
                    #         st.rerun()
                    with c2:
