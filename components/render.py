...
# ✅ ĐÃ THÊM import save_current_chat và load_user_chats
...
def init_food_state():
    ...
    # ✅ Tự động tạo chat mới nếu chưa có current_chat_id
...
def render_map_sidebar(map_col=None, restaurant_places=None):
    ...
    # **TỰ ĐỘNG RESET KHI RESTAURANT THAY ĐỔI (CHỈ NẾU ĐÃ CÓ ROUTE)**
    check_and_reset_on_restaurant_change(restaurant_places)

    # Render toàn bộ UI map trong cột map_col
    with map_col:
        # Tiêu đề chính
        st.markdown("## 🗺️ Route")
...
def check_and_reset_on_restaurant_change(restaurant_places):
    ...
    if previous_restaurants_ids != current_restaurants_ids: 
        # **LUÔN CẬP NHẬT FINGERPRINT TRƯỚC** để tránh recursion
        st.session_state.previous_restaurants_ids = current_restaurants_ids # dong nay la cap nhat
...
def get_restaurant_fingerprint(restaurant_places):
    ...
    """
    # Nếu danh sách nhà hàng trống hoặc None → trả về None
...
def render_restaurant_markers_map(restaurant_places):
    ...
    m = folium.Map(location=[10.776, 106.7], zoom_start=14)
