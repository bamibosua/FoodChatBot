import time          # Thư viện dùng để tạo độ trễ (sleep) giữa các request, tránh bị rate-limit
import requests      # Thư viện gửi HTTP request (GET, POST)
import folium        # Thư viện tạo bản đồ tương tác dựa trên Leaflet
import os            # Thư viện thao tác với hệ điều hành (đọc biến môi trường)
import polyline      # Thư viện decode encoded polyline (ORS trả về dạng chuỗi)
...
# OSRM = "http://localhost:5000"  # Dùng nếu bạn tự host OSRM

# User-Agent
# BẮT BUỘC với Nominatim, nếu không request sẽ bị từ chối
UA = {
...
def geocode(q):
    ...
    # Gửi request GET tới API Nominatim
    r = requests.get(
        f"{NOMINATIM}/search",
        params={
            "q": q,            # Chuỗi địa chỉ
            "format": "jsonv2",# Format JSON version 2
            "limit": 1         # Chỉ lấy kết quả đầu tiên
        },
        headers=UA             # Gửi User-Agent
    )

    # Nếu HTTP status code không phải 2xx -> raise exception
    r.raise_for_status()
...
def ors_routing(lon1, lat1, lon2, lat2):
    ...
    # Header cho ORS (bắt buộc có Authorization)
    headers = {
