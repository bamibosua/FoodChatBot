...
def postfilter(df, taste=None, budget=None, foods=None, original_location=None, is_fallback=False):
        ...
        if has_foods:
            ...
            for food_keyword in required_foods:
                if not re.search(rf'\b{re.escape(food_keyword)}\b', food_tag):
                    match_all = False
                    break
            foods_match = match_all
        
        if has_foods and has_taste:
            if not (foods_match and taste_match):
                if not is_fallback:
                    continue
        elif has_foods:
            if not foods_match:
    ...
    if "distance_km" in filtered.columns:
        filtered = filtered.sort_values(["category", "rating", "distance_km"], ascending=[True, False, True])
    else:
        filtered = filtered.sort_values(["category", "rating"], ascending=[True, False])

    return filtered

def fallback_expand_search(
    full_df,
    original_location,
    taste=None,
    budget=None,
    ...
    """Mở rộng kết quả khi số lượng < min_results.
       Bây giờ nó áp dụng cùng bộ lọc taste, foods, budget như postfilter."""
    ...
    open_status = df_open["open_hours"].apply(
        lambda t: is_open(t, current_time)
    )
    df_open["is_open"] = [x[0] for x in open_status]
    df_open["minutes_left"] = [x[1] for x in open_status]
    df_open = df_open[
        (df_open["is_open"]) & (df_open["minutes_left"] >= 30)
    ]
    if df_open.empty:
        return existing_results_df
    ...
    required_tags = set()
    has_taste = False
    if taste is not None and taste not in ("None", "none"):
        if isinstance(taste, str):
            clean = taste.strip()
            if clean and clean.lower() != "none":
                required_tags = {
            ...
            required_tags = {
                str(t).strip().lower()
                for t in taste
                if t and str(t).strip().lower() != "none"
            }
        has_taste = bool(required_tags)

    required_foods = None
    has_foods = False
    if foods is not None and foods not in ("None", "none"):
        if isinstance(foods, str):
            clean = str(foods).strip()
            if clean and clean.lower() != "none":
                required_foods = [clean.lower()]
            ...
            if cleaned:
                required_foods = cleaned
                has_foods = True

    parsed_budget = parse_price(budget)

    print("\n[FALLBACK FILTER DEBUG]")
    print(
        f"  has_taste: {has_taste} | tags: {required_tags if has_taste else 'N/A'}"
    )
    print(
        f"  has_foods: {has_foods} | food: {required_foods if has_foods else 'N/A'}"
    )
    print(f"  budget: {parsed_budget if parsed_budget else 'N/A'}")
    ...
    for _, row in candidates.iterrows():
        ...
        if has_taste:
            row_tags = str(row.get("taste_tags", "")).lower().strip()
            row_set = {
                t.strip() for t in row_tags.split(",") if t.strip()
            }
            taste_match = len(required_tags.intersection(row_set)) > 0
        ...
        if parsed_budget is not None:
            min_price = parse_price(str(row.get("budget", "")))
            if min_price is not None and min_price > parsed_budget:
                continue
    ...
    candidates = pd.DataFrame(filtered_candidates)
    ...
    candidates["category"] = "supplementary"
...
def filter_and_split_restaurants(
    full_df,
    location,
    taste=None,
    budget=None,
    ...
    """Quy trình toàn bộ: prefilter → postfilter → fallback (nếu cần) → split."""
    if current_time is None:
        current_time = datetime.now().strftime("%H:%M")

    main_results = prefilter(
        full_df, location=location, current_time=current_time
    )
    ...
    if not main_results.empty:
        main_results["rating"] = pd.to_numeric(
            main_results["rating"], errors="coerce"
        )
        main_results = main_results.sort_values(
            ["category", "rating", "distance_km"],
    ...
    if not final_results.empty:
        final_results["rating"] = pd.to_numeric(
            final_results["rating"], errors="coerce"
        )
    ...
    if len(final_results) > main_count:
        all_results = pd.concat(
            [main_results, final_results.iloc[len(main_results) :]],
            ignore_index=True,
        )
    else:
    ...
    if "category" not in all_results.columns:
        all_results["category"] = pd.NA

    main_list = (
        all_results[all_results["category"] == "main"]
        .to_dict("records")
    )
    supplementary_list = (
        all_results[all_results["category"] == "supplementary"]
        .to_dict("records")
    )
