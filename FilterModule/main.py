...
from filter_utils import prefilter, postfilter, fallback_expand_search, filter_and_split_restaurants

def main(intent_data, current_time=None, min_results=5):
    """
    ...
    # 7. Trả về 2 danh sách (không hiển thị)
    return main_restaurants, supplementary_restaurants

# Phần demo để test (có thể bỏ khi ghép module)
if __name__ == "__main__":
    ...
    main_list, supp_list = main(
        intent_data=demo_intent,
        current_time="11:30",
        min_results=5,
    )
    
    print(f"Found {len(main_list)} main restaurants and {len(supp_list)} supplementary restaurants")
